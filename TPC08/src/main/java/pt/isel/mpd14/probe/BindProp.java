        ...
        try {
            ...
            if (m != null) {
                ...
                if (propType.isAssignableFrom(v.getClass())) {
                    if(!formats.containsKey(m))
                        formats.put(m, m.getAnnotation(Format.class));
                    Format a = formats.get(m);
                    if(a!= null){
                        if(!formatters.containsKey(a))
                            formatters.put(a,a.formatter().newInstance());
                        v = formatters.get(a).format(v);
                    }
